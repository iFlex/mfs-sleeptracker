...
class Translator:
    ...
    def translate_sleep_record(self, data):
        ...
        sleep_times = data['sleep']
        for sleep in sleep_times:
            measurements = sleep['levels']['data']
            for measurement in measurements:
                field = {}
                field[measurement['level']] = float(measurement['seconds'])/60.0 #db uses minutes

                #record asleep periods
                if measurement['level'] in ['deep','light','rem']:
                    field['asleep'] = field[measurement['level']]
